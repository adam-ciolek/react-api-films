...
const SliderImage = () => {
  ...
  return (
    ...
    <Swiper
      ...
      autoplay={{delay:5000}}
      grabCursor={true} centeredSlides={true} slidesPerView={4}
      loop={true}
      // navigation={true}
      effect={'coverflow'}
      coverflowEffect={{
      ...
      >
      {slider.map(movie => {
        const {id,title,imDbRating,imDbRatingCount,year,image,crew} = movie;
        return (
          <SwiperSlide key={id}>
            <div className='flex bg-darkbg p-7 '>
              <img src={image} alt='' className='rounded-md' style={{width:150}}/>
              <div className='ml-4'>
                <h3 className='text-lg'>{title}</h3>
                <h4 className='text-sm my-2'>({year})</h4>
                <div className='flex flex-row my-2'>
                  <span className='text-sm'>{imDbRating}/10</span>
                  <span className='text-sm ml-8'>{imDbRatingCount} votes</span>
                </div>
                <p className='text-sm'>{crew}</p>
                <Link to='/' className='mr-8 capitalize text-lg hover:text-blue-700 transition'>
                  more
                </Link>
